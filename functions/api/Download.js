...
export async function onRequestGet(context) {
  ...
  const selectedItem = url.searchParams.get("manual") === "1" && manualPayloadUri(item.Id)
    ? { ...item, PayloadUri: manualPayloadUri(item.Id) }
    : item;
  const response = await proxyItemDownload(selectedItem, context.request, context.fetch || fetch);
